  ...
  {}({}{}PE_ID); \\
    ...
    def generate_nsdfg_arguments(self, sdfg, cfg, dfg, state, node):
        ...
        for _, _, _, vconn, in_memlet in state.in_edges(node):
            ...
            ptrname = self.ptr(in_memlet.data, desc, sdfg, in_memlet.subset, is_write=False, ancestor=1)
            ...
            if isinstance(desc, dace.data.Array) and (desc.storage == dtypes.StorageType.FPGA_Global
                ...
                memlet_references.append(
                    self.emit_memlet_reference(sdfg, in_memlet, vconn, conntype=node.in_connectors[vconn]))

        for _, uconn, _, _, out_memlet in state.out_edges(node):
            if out_memlet.data is not None:
                desc = sdfg.arrays[out_memlet.data]
                ptrname = self.ptr(out_memlet.data, desc, sdfg, out_memlet.subset, is_write=True, ancestor=1)
                defined_type, defined_ctype = self._dispatcher.defined_vars.get(ptrname, 1)
                ...
                if isinstance(desc, dace.data.Array) and (desc.storage == dtypes.StorageType.FPGA_Global
                    ...
                    memlet_references.append(
                        self.emit_memlet_reference(sdfg, out_memlet, uconn, conntype=node.out_connectors[uconn]))
    ...
    def allocate_view(self, sdfg: dace.SDFG, cfg: ControlFlowRegion, dfg: SDFGState, state_id: int,
        ...
        if nodedesc.storage == dace.dtypes.StorageType.FPGA_Global:
            ...
            eptr = self.ptr(edge.data.data, viewed_desc, sdfg)
    ...
    def generate_memlet_definition(
        ...
        data_name = self.ptr(data_name, data_desc, sdfg, memlet.subset, is_write=is_output)
...
{signature} {{
    ...
    def write_and_resolve_expr(self, sdfg, memlet, nc, outname, inname, indices=None, dtype=None):
        ...
        ptrname = self.ptr(memlet.data, desc, sdfg, memlet.subset, is_write=True)
